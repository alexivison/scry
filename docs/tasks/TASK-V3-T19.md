...
## Deliverables
- [ ] `ctrl+e` copies flagged paths to clipboard.
- [ ] Clipboard abstraction detects available tool and uses it.
- [ ] Unsupported environments show clear status bar error, no crash.
- [ ] Format is simple one-path-per-line, trivially pipeable.
- [ ] Empty flagged set shows "No flagged files" in status bar.
