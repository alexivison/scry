...
## Deliverables
- [ ] `--watch` starts periodic fingerprint checks after initial bootstrap.
- [ ] Stable fingerprint causes no churn.
- [ ] Changed fingerprint triggers the same shared refresh path as `r`.
- [ ] Help/status text exposes watch mode, interval, and last refresh/check timestamps.
- [ ] Watch loop stops cleanly on quit.
