...
func (m *Model) restoreNoteMutationView() bool {
	...
	if view.notes {
		if m.inactiveNoteCount() == 0 {
			m.setFileTreeCursor(m.State.FileTreeCursor)
			return true
		}
		width, height := m.inactiveNoteListSize()
		base, _ := panes.NoteListOffset(m.inactiveNotes(), m.noteState.selectedID, width)
		listOffset := min(max(view.listOffset, 0), panes.NoteListMaxOffset(m.inactiveNotes(), m.noteState.selectedID, width, height))
